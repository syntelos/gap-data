...
import gap.jac.annotation.processing.ProcessingEnvironment;
...
import gap.jac.source.tree.ClassTree;
import gap.jac.source.tree.CompilationUnitTree;
import gap.jac.source.tree.MethodTree;
import gap.jac.source.tree.Scope;
...
public abstract class Trees {
    ...
    public static Trees instance(CompilationTask task) {
        if (!task.getClass().getName().equals("gap.jac.api.JavacTaskImpl"))
    ...
    public static Trees instance(ProcessingEnvironment env) {
        if (!env.getClass().getName().equals("gap.jac.processing.JavacProcessingEnvironment"))
            throw new IllegalArgumentException();
    ...
    private static Trees getJavacTrees(Class<?> argType, Object arg) {
        try {
            ...
            Class<?> c = Class.forName("gap.jac.api.JavacTrees", false, cl);
